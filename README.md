# Eagle — 猎头 Agentic System

专为猎头设计的 Agentic AI 系统。通过自然语言对话管理招聘项目、自动评估候选人匹配度、执行行业调研，配合 Chrome 插件从招聘平台一键采集候选人。

> **本地优先**：所有数据存储在本地（SQLite + LanceDB），无需外部数据库服务。
...
| Chat（CA 对话） | 候选人评估 |
|---|---|
| ![Chat](docs/screenshots/chat.png) | ![Evaluation](docs/screenshots/evaluation.png) |

| 人才池 | Chrome 插件 |
|---|---|
| ![Talent Pool](docs/screenshots/talent-pool.png) | ![Extension](docs/screenshots/extension.png) |
...
- **Chrome 插件**：在 招聘平台页面一键采集候选人简历到本地人才池
- **支持 OpenAI / Anthropic**：通过 `LLM_PROVIDER` 切换（目前仅测试了OpenAI SDK）
